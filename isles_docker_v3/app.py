...
Grand Challenge runs your algorithm as an HTTP server with two endpoints:
  GET  /health  — Polled repeatedly after container start.
  POST /invoke  — Called per test case to process inputs.
"""
...
def _load_model_worker():
    """Load models in background thread so the HTTP server binds port 4743 immediately."""
    ...
    try:
        ...
        with MODEL_LOCK:
            MODEL = loaded
        _log(f"Background worker: Models loaded successfully in {time.time() - t0:.2f}s.")
